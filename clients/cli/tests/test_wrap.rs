...
async fn test_wrap_with_multisig() {
    ...
    // Create an unwrapped mint
    let unwrapped_mint = create_unwrapped_mint(&env, &unwrapped_token_program).await;

    // Initialize the wrapped mint derived from the unwrapped mint
    execute_create_mint(
        &env,
        &unwrapped_mint,
        &unwrapped_token_program,
    ...
    // Create an unwrapped token account owned by the multisig account
    let unwrapped_token_account = create_token_account(
        &env,
        &unwrapped_token_program,
        &unwrapped_mint,
    ...
    // Mint tokens to the unwrapped token account
    let starting_amount = 100;
    mint_to(
        &env,
        &unwrapped_token_program,
    ...
    // Set up the recipient account for wrapped tokens
    let wrapped_mint = get_wrapped_mint_address(&unwrapped_mint, &wrapped_token_program);
    let recipient_account =
        create_associated_token_account(&env, &wrapped_token_program, &wrapped_mint).await;
    ...
    // Execute the wrap transaction with multisig authorization
    let wrap_amount = 50;
